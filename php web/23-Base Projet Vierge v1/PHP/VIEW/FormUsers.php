...
?>
    ...
    <div>
        ...
        <input name="nomUser" <?php if ($mode !="ajout"&&$mode!="creer") { echo 'value="'.$userChoisi->getNomUser().'"';} if ($mode=="suppr" || $mode=="detail") echo 'disabled'; ?>/>
    </div>
    <div class="espace"></div>
     <div>
     <label for="prenomUser">Prenom</label>
     <input name="prenomUser"  <?php if ($mode !="ajout"&&$mode!="creer") { echo 'value="'.$userChoisi->getPrenomUser().'"';} if ($mode=="suppr" || $mode=="detail") echo 'disabled'; ?> />
     </div>
     <div class="espace"></div>
     <div>
     <label for="pseudoUser">Pseudo</label>
     <input name="pseudoUser"  <?php if ($mode !="ajout"&&$mode!="creer") { echo 'value="'.$userChoisi->getPseudoUser().'"';} if ($mode=="suppr" || $mode=="detail") echo 'disabled'; ?> />
     </div>
     <div class="espace"></div>
     <div>
         <label for="mailUser">Mail</label>
         <input name="mailUser" <?php if ($mode !="ajout"&&$mode!="creer") { echo 'value="'.$userChoisi->getMailUser().'"';} if ($mode=="suppr" || $mode=="detail") echo 'disabled'; ?> />
     </div>
     <div class="espace"></div>
     <div>
         <label for="passwordUser">Mot de passe</label>
         <input name="passwordUser" type="password"<?php if ($mode !="ajout"&&$mode!="creer") { echo 'value="'.$userChoisi->getPasswordUser().'"'; } if ($mode=="suppr" || $mode=="detail") echo 'disabled'; ?>  />
     </div>
    <?php
    ...
    <div>
            ...
            <select name="roleUser" <?php if ($mode=="suppr" || $mode=="detail") echo 'disabled';?>>
            <option value="1"<?php if (($mode=="suppr" || $mode=="detail")){if($userChoisi->getRole()==1){echo 'selected';}}?>>utilisateur</option>
